...
import { mockChallenges, sportColors } from '../services/mockData';
...
export default function ChallengesPage() {
  ...
  const [form, setForm] = useState({ title: '', type: 'Running', metric: 'distance', goal: '', unit: 'km', endDate: '' });
  ...
  const createChallenge = (e) => {
    e.preventDefault();
    const newC = { ...form, id: Date.now(), goal: Number(form.goal), current: 0, participants: 1, joined: true, leaderboard: [] };
            ...
            {challenges.map(c => {
              ...
              return (
                <div key={c.id} className="challenge-card" onClick={() => setSelected(c)}
                  style={{ borderColor: selected?.id === c.id ? 'var(--primary)' : 'var(--border)' }}>
